...
const HireChannel = ({ isDark, t }) => {
  ...
  || `Hola {{name}}, ¡gracias por escribir! Ya recibimos tu mensaje y lo responderemos a la brevedad.`;
        ...
        <form
          onSubmit={async (e) => {
            ...
            try {
              ...
              if (!freshEmail) {
                setSendStatus('error')
                setAuthError(t?.aboutPage?.muttForm?.auth?.cannotGetEmail || 'No se pudo autenticar el usuario. Intentalo de nuevo o usa la redirección.')
                setAuthCardStatus('error')
                return
              }
              ...
              try {
                ...
                setSendStatus('error')
                setAuthError(t?.aboutPage?.muttForm?.status?.error || 'Error al enviar el mensaje. Intenta de nuevo.')
                setAuthCardStatus('error')
              } finally {
                setSending(false)
                setAuthCardStatus('idle')
              ...
              if (error?.error === 'popup_closed' || error?.code === 'popup_closed') {
                setAuthError(t?.aboutPage?.muttForm?.auth?.popupClosed || 'Autenticación cancelada. Vuelve a intentar o usa redirección.')
              } else if (error?.error === 'popup_blocked' || error?.code === 'popup_blocked') {
                setAuthError(t?.aboutPage?.muttForm?.auth?.popupBlocked || 'El navegador bloqueó el popup. Habilita pop-ups o usa redirección.')
              } else {
                setAuthError(error?.message || (t?.aboutPage?.muttForm?.auth?.authenticating || 'No se pudo abrir el popup de autenticación.'))
              }
              setAuthCardStatus('error')
              setSendStatus('error')
            }
        ...
        >
          ...
          <div className={`text-lg font-semibold border-y ${isDark ? 'border-white/20' : 'border-black/20'} py-2 space-y-2`}>
            <div className="flex items-center gap-2">
              <span className="w-16 text-lg font-semibold">{t?.aboutPage?.muttForm?.to || 'Para'} {':'}</span>
              <span
                className={`flex-1 bg-transparent outline-none placeholder-current text-lg font-semibold`}>
                {t?.aboutPage?.profile?.name || 'Flavio Gabriel Morales'}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-18 text-lg whitespace-nowrap">{t?.aboutPage?.muttForm?.from || 'De'} {':'}</span>
          ...
          <div className={`mt-3 text-sm`}>{t?.aboutPage?.muttForm?.labels?.message || '-- Mensaje --'}</div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="relative">
              <button
                ...
                `}
              >
                <span className="relative z-10 whitespace-nowrap">{sending ? (t?.aboutPage?.muttForm?.sending || 'Enviando...') : (t?.aboutPage?.muttForm?.submit || 'Enviar ↗')}</span>
              </button>
              {sendStatus === 'success' && (
                <p className={`mt-2 text-xs whitespace-nowrap ${isDark ? 'text-white/70' : 'text-black/70'}`}>{(t?.aboutPage?.muttForm?.status?.success || '¡Mensaje enviado con éxito!').replace('{{email}}', lastSentEmail || '')}</p>
              )}
              {sendStatus === 'error' && (
                <p className={`mt-2 text-xs whitespace-nowrap ${isDark ? 'text-white/70' : 'text-black/70'}`}>{t?.aboutPage?.muttForm?.status?.error || 'Error al enviar el mensaje. Por favor, intenta de nuevo.'}</p>
              )}
            </div>
        ...
        <div className="absolute inset-0 z-0 flex items-center justify-center">
          ...
          <div className={`relative w-[90%] max-w-md p-4 border  rounded-md ${isDark ? 'bg-primary text-white border-white/20' : 'bg-white text-black border-black/20'}`}>
            <div className={`text-sm border-b ${isDark ? 'border-white/20' : 'border-black/20'} pb-2 mb-3 flex items-center justify-between`}>
              <span className="font-bold">{t?.aboutPage?.muttForm?.auth?.title || '[ autenticación ]'}</span>
              <button
                type="button"
            ...
            {authCardStatus === 'authenticating' && (
              <p className="text-xs opacity-80">{t?.aboutPage?.muttForm?.auth?.authenticating || 'Autenticando... completa la verificación para continuar.'}</p>
            )}
