...
import intro from '../../assets/introbg.webp'
import scenebg from '../../assets/scenebg.png'
import scene from '../../assets/scene.webp'
import zondaIcon from '../../assets/zonda.webp'
import palitoIcon from '../../assets/palito.webp'
import sauce from '../../assets/sauce.webp'
import pino from '../../assets/pino.webp'
import menu from '../../assets/menu.webp'
import zonda from '../../assets/zonda_item.gif'
import palito from '../../assets/palito_item.gif'
import aguilucho from '../../assets/aguilucho.webp'
...
const PlayChannel = ({ theme, language, isDark: isDarkProp, t: tProp }) => {
  ...
  const resetGame = useCallback(() => {
    ...
    controlsRef.current = { jumpHeld: false, duckHeld: false }
...
██      ██   ██    ██     ██████  ██   ████ ██ ██   ██
  ...
  // Colisión círculo-rectángulo: para 'redondear' la hitbox real del personaje
  ...
  const spawnObstacle = (forcedType) => {
    ...
    const wb = 10  // ancho pino
    const hb = 36  // alto pino
  ...
  useEffect(() => {
    ...
    const tick = (ts) => {
      ...
      // Colisión usando hitbox circular del personaje contra rectángulos de obstáculos
      const cRect = getCharRect(char)
      const cx = cRect.left + cRect.width / 2
      const cy = cRect.bottom + cRect.height / 2
      ...
      const collided = collisionsOn && obstacles.some(o => {
        ...
        return circleIntersectsRect(cx, cy, radius, oRect)
      })
  ...
  return (
    <div className="w-full h-full font-arcade">
      ...
      >
        ...
        {/* Obstáculos */}
        {obstacles.map(o => {
          // Si es un obstáculo del techo: renderizar con asset `aguilucho`
          if (o.type === 'ceiling') {
            const r = getObstacleRect(o)
            return (
          ...
          // Obstáculos del piso: renderizar con asset `pino`
          if (o.type === 'floor') {
            const r = getObstacleRect(o)
            return (
            <div key={`${o.id}-container`}>
        ...
        {showHitboxes && DEBUG_MODE && (() => { const r = getCharRect(char); return (
          <div
            className="absolute border-2 border-green-500 bg-green-500/20 rounded-full pointer-events-none"
            style={{
              left: `${r.left}%`,
              bottom: `${r.bottom}%`,
              width: `${r.width}%`,
        ...
        {obstacles.map(o => {
          ...
          return (
            ...
            >
              ...
              style={{filter: isDark ? 'invert(1)' : 'invert(1)'}}
              />
            </div>
          )
        })}
        ...
        {state === 'menu' && (
          ...
          >
            ...
            <div className="text-xs pt-20 mt-2 text-secondary">{t?.playChannel?.menu?.helper}</div>
            <div className="text-xs text-secondary">{t?.playChannel?.menu?.power}</div>
            <button
              onClick={() => { if (assetsReady) { resetGame(); setState('playing') } }}
              disabled={!assetsReady}
              className={`inline-flex items-center gap-2 px-3 py-1 rounded border border-secondary text-secondary text-xl hover:bg-cloud hover:text-void ${!assetsReady ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
