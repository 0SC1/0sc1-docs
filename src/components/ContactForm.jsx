...
export default function ContactForm({ isDark, onCardOpenChange, t }) {
  ...
  const [invalidMsgKey, setInvalidMsgKey] = useState(null) // e.g. 'required'

  // Auth card UI
  const [showAuthCard, setShowAuthCard] = useState(false)
  const [authCardStatus, setAuthCardStatus] = useState('idle') // 'idle' | 'authenticating' | 'error'
  const [authError, setAuthError] = useState('')
  const [authErrorKey, setAuthErrorKey] = useState(null) // e.g. 'cannotGetEmail', 'cannotGetEmailAfterRedirect', 'popupClosed', 'popupBlocked', 'redirectError'
  ...
  const base = t?.aboutPage?.muttForm?.userMessage
  ...
  const sendClientReceipt = async ({ toEmail, toName }) => {
    ...
    const pub = import.meta.env.VITE_CLIENT_EMAILJS_PUBLIC_KEY || import.meta.env.
    VITE_CLIENT_EMAILJS__PUBLIC_KEY
    const message = base.replace('{{name}}', toName || 'amig@')
    if (!svc || !tpl || !pub) return
    const params = {
      // El template espera: Subject: {{subject}}, To Email: {{email}}, Content: {{message}}
      email: toEmail,
      subject: (t?.aboutPage?.muttForm?.subject || 'Recibimos tu mensaje'),
      message: (message),
    }
    try {
    ...
    <div className="space-y-4 font-sans text-lg">
          ...
          try {
            ...
            if (isRedirectPending) {
              ...
              if (!freshEmail) {
                ...
                setAuthErrorKey('cannotGetEmailAfterRedirect')
            ...
            if (!freshEmail) {
              ...
              setAuthErrorKey('cannotGetEmail')
              setAuthCardStatus('error')
              return
            }
      ...
      noValidate>
        <div className="relative">
          <label htmlFor="name" className="block text-md font-bold mb-2 font-sans">
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => {
                ...
                if (invalidField === 'name' && e.target.value.trim()) { setInvalidField(null); setInvalidMsg(''); setInvalidMsgKey(null) }
              }}
              className={`w-full p-2 rounded-lg ${isDark ? 'bg-void' : 'bg-cloud'} font-sans ${invalidField === 'name' ? 'pr-36' : ''} focus:outline-none focus:ring-0`}
            ...
            />
            {invalidField === 'name' && (
              <div id="error-name" className={`absolute top-2 right-2 pointer-events-none select-none text-md font-sans rounded-md px-2.5 py-1 flex items-center gap-1 z-10 whitespace-nowrap ${isDark ? 'bg-void text-white' : 'bg-cloud text-black'}`}>
                <span>{(invalidMsgKey ? (t?.aboutPage?.muttForm?.status?.[invalidMsgKey]) : null) || invalidMsg || 'Rellene este campo'}</span>
        ...
        <div className="relative">
          <label htmlFor="message" className="block text-md font-bold mb-2 font-sans">
          ...
          <div className="relative">
            <textarea
              ...
              onChange={(e) => {
                ...
                if (invalidField === 'message' && e.target.value.trim()) { setInvalidField(null); setInvalidMsg(''); setInvalidMsgKey(null) }
              }}
              className={`w-full p-2 rounded-lg ${isDark ? 'bg-void' : 'bg-cloud'} font-sans ${invalidField === 'message' ? 'pr-36' : ''} focus:outline-none focus:ring-0`}
            ...
            {invalidField === 'message' && (
              <div id="error-message" className={`absolute top-2 right-2 pointer-events-none select-none text-md font-sans rounded-md px-2.5 py-1 flex items-center gap-1 z-10 whitespace-nowrap ${isDark ? 'bg-void text-white' : 'bg-cloud text-black'}`}>
                <span>{(invalidMsgKey ? (t?.aboutPage?.muttForm?.status?.[invalidMsgKey]) : null) || invalidMsg || 'Rellene este campo'}</span>
                <Icon icon="mdi:alert" width="16" height="16" />
              </div>
            )}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {sendStatus === 'success' && (
            <p className="text-feather text-md ">{(t?.aboutPage?.muttForm?.status?.success || '¡Mensaje enviado con éxito desde {{email}}!').replace('{{email}}', lastSentEmail || '')}</p>
          )}
          {sendStatus === 'error' && (
            <p className="text-red-500 text-md ">{t?.aboutPage?.muttForm?.status?.error || 'Error al enviar el mensaje. Por favor, intenta de nuevo.'}</p>
          )}
        </div>
      </form>
      ...
      {showAuthCard && (
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none ">
          <div
            className={`pointer-events-auto rounded-xl shadow-lg w-[90%] max-w-md p-5 relative ${isDark ? 'bg-primary' : 'bg-secondary'}`}
          >
            {/* Header */}
            <div className={`mb-3 pb-2 border-b flex items-center justify-between ${isDark ? 'border-white/15' : 'border-black/15'}`}>
              <h4 className="font-sans font-bold text-lg">{t?.aboutPage?.muttForm?.auth?.title || 'Autenticación'}</h4>
              <button
                type="button"
                aria-label={t?.aboutPage?.muttForm?.auth?.closeAria || 'Cerrar'}
                className={`relative isolate overflow-hidden h-8 w-8 flex items-center justify-center rounded-lg text-md font-bold
                  transition-colors duration-300
                  before:content-[''] before:absolute before:inset-0 before:rounded-full
                  before:scale-0 hover:before:scale-150 before:transition-transform before:duration-300 before:ease-out before:origin-[var(--ox)_var(--oy)]
                  ${isDark ? 'bg-void text-white hover:text-primary before:bg-cloud' : 'bg-cloud text-primary hover:text-secondary before:bg-primary'}`}
            ...
            {authCardStatus === 'authenticating' && (
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <p className="font-sans text-md opacity-80 text-center">{t?.aboutPage?.muttForm?.auth?.authenticating || 'Autenticando... completa el popup para continuar.'}</p>
              </div>
            )}
            {authCardStatus === 'error' && (
              <div className="space-y-2 py-1">
                <p className="font-sans text-md text-error leading-snug whitespace-normal">{authErrorKey ? (t?.aboutPage?.muttForm?.auth?.[authErrorKey] || authError) : authError}</p>
              </div>
            )}
            <div className="flex gap-2">
              <button
                type="button"
                className={`relative isolate overflow-hidden px-3 py-2 rounded-lg text-xs font-bold
                  transition-colors duration-300
                  before:content-[''] before:absolute before:inset-0 before:rounded-full
                  before:scale-0 hover:before:scale-150 before:transition-transform before:duration-300 before:ease-out before:origin-[var(--ox)_var(--oy)]
                  ${isDark ? 'bg-void text-white hover:text-primary before:bg-cloud' : 'bg-cloud text-primary hover:text-secondary before:bg-primary'}`}
              ...
              >
                <span className="relative z-10 whitespace-nowrap">{t?.aboutPage?.muttForm?.auth?.retry || 'Reintentar'}</span>
              </button>
              <button
                type="button"
                className={`relative isolate overflow-hidden px-3 py-2 rounded-lg text-xs font-bold
              ...
              >
                <span className="relative z-10 whitespace-nowrap">{t?.aboutPage?.muttForm?.auth?.useRedirect || 'Usar redirección'}</span>
              </button>
            </div>
          </div>
        </div>
