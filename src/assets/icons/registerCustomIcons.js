...
import blacksunRaw from '../BlackSunLogo.svg?raw'
import horneroRaw from '../HorneroLogo.svg?raw'
import osciRaw from '../osci.svg?raw'
...
export default function registerCustomIcons() {
  try {
    ...
    if (typeof osciRaw === 'string' && osciRaw) {
      const m = osciRaw.match(/viewBox="\s*0\s+0\s+([\d.]+)\s+([\d.]+)\s*"/i)
      const width = m ? parseFloat(m[1]) : 24
      const height = m ? parseFloat(m[2]) : 24
      const inner = osciRaw
        .replace(/^[\s\S]*?<svg[^>]*>/i, '')
        .replace(/<\/svg>\s*$/i, '')
      addIcon('custom:osci', { body: inner, width, height })
    }
  } catch (e) {
