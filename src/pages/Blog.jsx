...
function Blog() {
  ...
  const [lastToggledSlug, setLastToggledSlug] = useState("")
  const [preOpenSlug, setPreOpenSlug] = useState("")
  const [skipOpenSlideSlug, setSkipOpenSlideSlug] = useState("")
  ...
  const toggleExpanded = (slug) => {
    ...
    const sp = new URLSearchParams(location.search)
    // Record which card initiated the toggle (open or close)
    setLastToggledSlug(slug)
    if (isOpen) {
      ...
      // Animate list scroll so the selected card reaches the top, then pre-slide icon and finally expand
      const container = document.getElementById('post-list')
      const card = document.getElementById(`card-${slug}`)
      const expandNow = () => {
        // Start a pre-opening slide in the card before actually expanding
        setPreOpenSlug(slug)
        const SLIDE_MS = 280
        setTimeout(() => {
          // signal PostCard to skip the internal opening slide
          setSkipOpenSlideSlug(slug)
          sp.set('post', slug)
          navigate({ search: sp.toString() ? `?${sp.toString()}` : "" }, { replace: true })
          setExpandedSlug(slug)
          // clear pre-open flag after expansion is scheduled
          setPreOpenSlug("")
          // Remove any temporary bottom spacer used for autoscroll near the end
          try {
            const cont = document.getElementById('post-list')
            const spEl = document.getElementById('post-list-spacer')
            if (cont && spEl && spEl.parentNode === cont) cont.removeChild(spEl)
          } catch (_) { /* noop */ }
        }, SLIDE_MS)
      }
      ...
      // If desired goes beyond max scrollable range (near the end), add a temporary spacer
      try {
        const maxScroll = container.scrollHeight - container.clientHeight
        if (desired > maxScroll) {
          const need = Math.ceil(desired - maxScroll + 8) // small buffer
          let spEl = document.getElementById('post-list-spacer')
          if (!spEl) {
            spEl = document.createElement('div')
            spEl.id = 'post-list-spacer'
            spEl.style.width = '1px'
            spEl.style.pointerEvents = 'none'
            container.appendChild(spEl)
          }
          spEl.style.height = `${need}px`
        }
      } catch (_) { /* noop */ }
      const threshold = 2
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          return (
            <div key={post.slug} >
              <PostCard
                ...
                triggerSlug={lastToggledSlug}
                preOpenSlug={preOpenSlug}
                skipOpenSlideSlug={skipOpenSlideSlug}
      ...
      <div aria-live="polite" className="sr-only">
         {copiedSlug ? (t?.md?.postLink?.copied || 'Link copied') : ''}
      </div>
    </div>
  )
}
